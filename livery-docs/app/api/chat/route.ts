...
function createStudioInstructions(currentSource: string, theme: BuiltInThemeName, latestUserRequest: string) {
  return [
    ...
    'Use grid, row, or column only when the user explicitly requests exact composition. grid requires columns and accepts gap, width, height, align, and distribute; it does not accept columnGap or rowGap. Never force sequential stages into a 2×2 grid.',
    ...
    'Keep feedback and return connectors local to the nodes they connect. Never route a connector around the outer canvas merely to imply iteration or a response; use a short local loop, a subtitle, or omit a redundant return edge.',
    ...
    'When top-level stage frames are stacked vertically, keep each frame shallow: arrange two or three members in a short row unless the user explicitly requires those members themselves to be vertical. For a compact peer-frame grid, a three- or four-member frame may use its own two-column grid so the outer grid remains within the canvas. Do not interpret vertical frame order as a request for columns inside every frame.',
